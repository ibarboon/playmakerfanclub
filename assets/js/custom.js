...
if($('#videoBg').length){
	...
	$(document).on('submit', '#register-form', function() {
		...
		if($('[name=input-password]').val() !== $('[name=input-confirm-password]').val()) {
			$('[name=input-confirm-password]').val(null).focus();
			formValidate = false;
		}
		return formValidate;
	});

	var urlGetSliderList = $('#input-slider-list').val(), sliderList;
	$.ajax({
		url: urlGetSliderList,
		dataType: "json",
...
function initialize(id) {
	...
	var overlayTitle = 'Play Maker Fan Club';

	var locations = [['Play Maker Fan Club', 'Bangkok', 13.839314460754395, 100.621566772460940]];
...
$(document).ready(function(){
	$('#load-more-promotion').on('click', function(){
		$('.row.mb15').clone().insertBefore($(this));
		$(this).hide();
	});
});